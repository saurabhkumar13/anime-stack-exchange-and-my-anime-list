...
-- initializing means
    ...
    loop
        ...
        insert into test_norm_means (select att,i+1,data,0 from test_norm where att<>-1 and id=misc2);
...
-- Break Condition
--exit when (1=1);
    select max((a.mean-b.mean)*(a.mean-b.mean)) into miscf from (select att,class,mean from test_norm_means where ol=0) a,(select att,class,mean from test_norm_means where ol=1) b where a.class=b.class and a.att=b.att;
